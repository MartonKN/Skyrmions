function cMap=colormapDensities()
% Jet / green->white
cMap=jet(500);
x=linspace(0,1,floor(length(cMap)/3))';
tmp=[x*cMap(1,1),x*cMap(1,2),0.2*(1-x)+x*cMap(1,3)];
cMap=[tmp;cMap];
tmp=[0.2*x+(1-x)*cMap(length(cMap),1),(1-x)*cMap(length(cMap),2),(1-x)*cMap(length(cMap),3)];
cMap=[cMap;tmp];
